function send(query) {
    ...
    fetch("http://127.0.0.1/predict", requestOptions)
        ...
        .then(function(result) {
            ...
            setTimeout(() => {
                ...
                if (localStorage.last_score) {
                    ...
                    var final_score =
                        (parseFloat(localStorage.last_score) +
                            current_score * localStorage.w) /
                        2;
                ...
                if (parseFloat(localStorage.last_score) >= 0.8) {
                    localStorage.setItem("w", 1.0);
...
function mails() {
    ...
    chrome.storage.sync.get("email", function(items) {
        ...
        fetch("http://127.0.0.1/email", requestOptions)
            .then((response) => response.text())
            .then(function(result) {
                setTimeout(() => {}, 1000);
            })
